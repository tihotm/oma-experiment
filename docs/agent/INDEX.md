# Agent Index

`docs/` is the system of record. This file is the navigation map.

Read order for a new task:

1. [CURRENT-WORK.md](CURRENT-WORK.md)
2. [../CURRENT-STATE.md](../CURRENT-STATE.md)
3. [../SPEC.md](../SPEC.md)
4. [../INVARIANTS.md](../INVARIANTS.md)
5. [../LIFECYCLE.md](../LIFECYCLE.md)
6. [../CLAIM-MAP.md](../CLAIM-MAP.md)
7. [../ROADMAP.md](../ROADMAP.md)

Specialized contracts:

- [OPERATING-CONTRACT.md](OPERATING-CONTRACT.md)
- [SCIENTIFIC-CONTRACT.md](SCIENTIFIC-CONTRACT.md)
- [SUPERVISION-CONTRACT.md](SUPERVISION-CONTRACT.md)
- [DOCKER-CONTRACT.md](DOCKER-CONTRACT.md)
- [VERIFICATION-CONTRACT.md](VERIFICATION-CONTRACT.md)
- [TEST-CONTRACT.md](TEST-CONTRACT.md)

Versioned state:

- [CURRENT-WORK.md](CURRENT-WORK.md) - current factual state and active blockers
- [FIRST-REAL-MISSION.md](FIRST-REAL-MISSION.md) - canonical first real mission and release-candidate shape
- [../CURRENT-STATE.md](../CURRENT-STATE.md) - stable repository state and counters
- [../DECISIONS.md](../DECISIONS.md) - architectural decisions
- [../ROADMAP.md](../ROADMAP.md) - active roadmap and staged scope
- [../CURRENT-CONTRACT-GAP-AUDIT.md](../CURRENT-CONTRACT-GAP-AUDIT.md) - historical contract gap evidence

Validation helpers:

- [../../scripts/validate-harness.py](../../scripts/validate-harness.py)
